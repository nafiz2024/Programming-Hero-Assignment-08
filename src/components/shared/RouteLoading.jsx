const shimmerCard = "overflow-hidden rounded-[1.75rem] border border-white/70 bg-white/80 shadow-[0_24px_80px_rgba(15,23,42,0.10)] backdrop-blur";
...
const LoadingDots = ({ light = false }) => {
    ...
    return (
        <div className="flex items-center gap-2" aria-label="Loading">
            <span className={`h-2.5 w-2.5 rounded-full ${dotColor} animate-bounce [animation-delay:-0.3s]`} />
            <span className={`h-2.5 w-2.5 rounded-full ${dotColor} animate-bounce [animation-delay:-0.15s]`} />
...
export const MainRouteLoading = () => {
    return (
        <section className="animate-pulse px-3 pb-8 pt-2 sm:px-5 sm:pt-3 lg:px-0">
            <div className={`${shimmerCard} bg-gradient-to-br from-white via-slate-50 to-sky-50 px-4 py-6 sm:px-6 sm:py-8 lg:px-10 lg:py-10 xl:px-16 xl:py-14`}>
                <div className="grid items-center gap-6 lg:grid-cols-[minmax(0,1.02fr)_minmax(320px,0.98fr)] lg:gap-8 xl:gap-12">
                    <div>
                        <div className="mb-5 inline-flex items-center gap-3 rounded-full bg-white/80 px-4 py-2 shadow-sm">
                            <LoadingDots />
                            <span className="text-xs font-semibold uppercase tracking-[0.24em] text-emerald-600">Loading</span>
                        </div>
                        ...
                        <div className="mt-8 grid gap-3 rounded-[1.75rem] border border-slate-200/70 bg-white/90 p-4 sm:grid-cols-3 sm:gap-4 sm:p-5">
                            {[1, 2, 3].map((item) => (
                                <div key={item} className="flex items-center gap-3 border-b border-slate-100 pb-3 last:border-b-0 last:pb-0 sm:block sm:border-b-0 sm:border-r sm:pb-0 sm:last:border-r-0 sm:pr-4">
                    ...
                    <div className="flex flex-col justify-between gap-8 bg-[linear-gradient(160deg,_rgba(16,185,129,0.96),_rgba(5,150,105,0.92))] p-6 sm:p-8 lg:p-10">
                        <div>
                            <div className="mb-5 inline-flex items-center gap-3 rounded-full bg-white/10 px-4 py-2 shadow-sm">
                                <LoadingDots light />
                                <span className="text-xs font-semibold uppercase tracking-[0.24em] text-white/90">Loading</span>
                            </div>
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            {[1, 2].map((item) => (
                                <div key={item} className="rounded-3xl border border-white/15 bg-white/10 p-4">
